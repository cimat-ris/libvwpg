...
KNOWN_TAGS = ('parameter', 'com_position', 'com_speed','com_theta','com_angular_speed',
              ...
              'camera_position','execution_time','mapping_execution_time','loop_closing_execution_time','good_matched_points','total_matched_points')
...
def plot_walk_pattern(all_raw_data, prefix_name):
    ...
    # l_obs = 0.242
    # h_obs = 0.662
    # axis.add_patch(patches.Rectangle((1.5 - l_obs/2 , -0.3 - h_obs/2), l_obs, h_obs, 0, fill=False, edgecolor='y', linewidth=4))
    # print("center = {}".format((1.5 - l_obs/2 , -0.3 - h_obs/2)))

    x_axis, y_axis, __ = prepare_data_for_plot(all_raw_data['com_position'])
    axis.plot(x_axis, y_axis, 'r', label='CoM', linewidth=graphwidth)

    # If camera postions are available
    if(len(all_raw_data['camera_position'])>0):
        x_axis, y_axis, phi = prepare_data_for_plot(all_raw_data['camera_position'])
        axis.plot(x_axis, y_axis, 'g', label='SLAM Camera', linewidth=graphwidth)
    ...
    #axis.set_ylim(-0.75,1.5)
    axis.set_ylim(-4.5,0.5)
    axis.set_xlim(-0.5,5.0)
    axis.set_yticks(np.arange(-4.5, 1.0, step=0.5))
    axis.set_xticks(np.arange(-0.5, 5.5, step=0.5))
...
def save_execution_time_values(all_raw_data, prefix_name):
    """ """
    units = all_raw_data['execution_time'][0].split()[1].strip() 
    values = [entry.split()[0] for entry in all_raw_data['execution_time']]
    values = [entry.split('=')[1] for entry in values]
    values = [int(entry) for entry in values]


    #print(values)
    with open((prefix_name+"_execution_time_values.txt"), "w") as f:
        for i in values:
            f.write(str(i))
            f.write('\n')

def print_matched_points(all_raw_data, prefix_name):     
    values = [entry.split()[0] for entry in all_raw_data['good_matched_points']]
    values = [entry.split('=')[1] for entry in values]
    values = [int(entry) for entry in values]
    print(values)
    print("mean good_matched_points = {}".format(sum(values)/len(values)))
    del(values)

    values = [entry.split()[0] for entry in all_raw_data['total_matched_points']]
    values = [entry.split('=')[1] for entry in values]
    values = [int(entry) for entry in values]
    print(values)
    print("mean total_matched_points = {}".format(sum(values)/len(values)))

def print_mapping_loopclosing_execution_time(all_raw_data, prefix_name):
    """ """
    units = all_raw_data['mapping_execution_time'][0].split()[1].strip() 
    values = [entry.split()[0] for entry in all_raw_data['mapping_execution_time']]
    values = [entry.split('=')[1] for entry in values]
    values = [float(entry) for entry in values]
    print(values)
    print("mean mapping time = {} {}".format(sum(values)/len(values),units))

    units = all_raw_data['loop_closing_execution_time'][0].split()[1].strip() 
    values = [entry.split()[0] for entry in all_raw_data['loop_closing_execution_time']]
    values = [entry.split('=')[1] for entry in values]
    values = [float(entry) for entry in values]
    print("mean loop-closing time = {} {}".format(sum(values)/len(values),units))   
...
def video_of_detailed_visual_features(all_raw_data, prefix_name):
    ...
    axis = figure.add_subplot(1, 1, 1)
    figure.set_size_inches(1.25*figure.get_size_inches())
    elements_vector_actual_values = []
    elements_vector_predicted_values = []
    data = 0
    for index, name in enumerate(parsed_data.keys()):
        pyplot.cla()
        actual_values, predicted_values, __ = parsed_data[name]
        elements_vector_actual_values.append(actual_values)
        elements_vector_predicted_values.append(predicted_values)

    try:
        __, __, expected_values_1, = parsed_data['h11']
        __, __, expected_values_2,  = parsed_data['h12']
    except KeyError:
        __, __, expected_values, __ = parsed_data['e12']
        axis.plot(expected_values, color='black', label='reference', linestyle='--', linewidth=graphwidth)
        axis.legend(loc='best',fontsize=legendfontsize-3,ncol=3)
    ...
    files = []
    for i in range(1,len(elements_vector_actual_values[0])):
        axis.plot(expected_values_1, color='black', label='reference', linestyle='--', linewidth=graphwidth)
        if(i==1):
            axis.legend(loc='best',fontsize=legendfontsize-3,ncol=3) 
        axis.plot(expected_values_2, color='black', label='reference', linestyle='--', linewidth=graphwidth)
        axis.grid(True, linewidth=graphwidth)
        axis.set_xlabel("Iterations",size=axisfontsize)
        axis.set_ylabel("Homography matrix elements",size=axisfontsize)
        axis.tick_params(labelsize=ticksfontsize,labelcolor='black',pad=3) 
        axis.set_ylim(-1.2,1.2)
        axis.plot(elements_vector_actual_values[0][0:i], color=colors[0], label=r'%s observed'%('h11'), linestyle='-', linewidth=graphwidth)
        axis.plot(elements_vector_predicted_values[0][0:i], color=colors[0], label=r'%s predicted'%('h11'), linestyle='--', linewidth=graphwidth)
        axis.plot(elements_vector_actual_values[1][0:i], color=colors[1], label=r'%s observed'%('h12'), linestyle='-', linewidth=graphwidth)
        axis.plot(elements_vector_predicted_values[1][0:i], color=colors[1], label=r'%s predicted'%('h12'), linestyle='--', linewidth=graphwidth)
        axis.plot(elements_vector_actual_values[2][0:i], color=colors[2], label=r'%s observed'%('h13'), linestyle='-', linewidth=graphwidth)
        axis.plot(elements_vector_predicted_values[2][0:i], color=colors[2], label=r'%s predicted'%('h13'), linestyle='--', linewidth=graphwidth)
        axis.plot(elements_vector_actual_values[3][0:i], color=colors[3], label=r'%s observed'%('h32'), linestyle='-', linewidth=graphwidth)
        axis.plot(elements_vector_predicted_values[3][0:i], color=colors[3], label=r'%s predicted'%('h32'), linestyle='--', linewidth=graphwidth)
        axis.plot(elements_vector_actual_values[4][0:i], color=colors[4], label=r'%s observed'%('h33'), linestyle='-', linewidth=graphwidth)
        axis.plot(elements_vector_predicted_values[4][0:i], color=colors[4], label=r'%s predicted'%('h33'), linestyle='--', linewidth=graphwidth)
        axis.plot(elements_vector_actual_values[5][0:i], color=colors[5], label=r'%s observed'%('h31'), linestyle='-', linewidth=graphwidth)
        axis.plot(elements_vector_predicted_values[5][0:i], color=colors[5], label=r'%s predicted'%('h31'), linestyle='--', linewidth=graphwidth)
        if(i==1):
            axis.legend(loc='best',fontsize=legendfontsize-3,ncol=3)  
    ...
    subprocess.call("mencoder 'mf://_tmp*.png' -mf type=png:fps=10 -ovc lavc "
                    "-lavcopts vcodec=wmv2 -oac copy -o {0}_visual_features_animation.mpg".format(prefix_name), shell=True)
...
def main():
    ...
    for log_file in sys.argv[1:]:
        ...
        #plot_functions = (plot_walk_pattern,print_matched_points,plot_detailed_visual_features)
        plot_functions = (plot_walk_pattern,plot_com_speed)
        #plot_functions = (plot_walk_pattern,print_mapping_loopclosing_execution_time)
        #plot_functions = (plot_walk_pattern,plot_camera_images,plot_com_acceleration)
        #plot_functions = (plot_walk_pattern,video_of_detailed_visual_features)
